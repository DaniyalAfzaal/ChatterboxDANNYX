...
class YamlConfigManager:
    ...
    def get_dict(self, key_path: str, default: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Retrieves a configuration value, ensuring it's a dictionary."""
        raw_value = self.get(key_path)
        if raw_value is None:
            if default is not None:
                logger.debug(
                ...
                return default
            logger.error(
                f"Mandatory dict config '{key_path}' is None, and no method default. Returning empty dict."
            )
            return {}
        if isinstance(raw_value, dict):
            return raw_value
        logger.warning(
            f"Invalid dict value '{raw_value}' for '{key_path}'. Using method default or empty dict."
        )
    ...
    def update(self, new_settings: Dict[str, Any]):
        ...
        This is expected by server.py.
        """
        with self._lock:
            _deep_merge_dicts(new_settings, self.config)
            # Re-resolve paths/device if needed
            self.config = self._resolve_paths_and_device(self.config)
...
# ---- Helper Functions for Server Compatibility ----
# These functions are expected by server.py and were added to fix ImportErrors.
...
def get_base_output_dir(cfg: Any = None) -> str:
    return str(get_output_path())

def get_server_host(cfg: Any = None) -> str:
    return get_host()

def get_server_port(cfg: Any = None) -> int:
    return get_port()

def get_device_setting(cfg: Any = None) -> str:
    return get_tts_device()

def get_default_split_text_setting(cfg: Any = None) -> bool:
    cfg_map = _unwrap_cfg(cfg)
    gen = cfg_map.get("generation_defaults") or {}
    if isinstance(gen, dict) and "split_text" in gen:
        return bool(gen["split_text"])
    if "split_text" in cfg_map:
        return bool(cfg_map["split_text"])
    return True

def get_default_chunk_size_setting(cfg: Any = None) -> int:
    cfg_map = _unwrap_cfg(cfg)
    gen = cfg_map.get("generation_defaults") or {}
    if isinstance(gen, dict) and "chunk_size" in gen:
        try:
...
def get_default_split_text(cfg: Any = None) -> bool:
    return get_default_split_text_setting(cfg)

def get_default_chunk_size(cfg: Any = None) -> int:
