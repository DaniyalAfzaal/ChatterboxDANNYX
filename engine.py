...
def synthesize(
    ...
    try:
        ...
        # Re-raise RuntimeError (includes CUDA errors) so server.py can handle them
        logger.error(f"RuntimeError during TTS synthesis: {e}", exc_info=True)
        raise
    except Exception as e:
        # Only catch and suppress non-RuntimeError exceptions
