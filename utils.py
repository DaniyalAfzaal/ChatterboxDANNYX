...
def sanitize_text(text: str) -> str:
    """
    Sanitizes input text by removing non-printable characters, normalizing whitespace,
    and aggressively filtering problematic Unicode that can cause tokenization errors.
    
    This helps prevent invalid token ID errors in the TTS model by ensuring only
    safe ASCII characters are passed to the tokenizer.
    
    Args:
        text: The text to sanitize.
    
    Returns:
        Sanitized text string with only safe ASCII characters.
    """
    if not text:
        return ""
    
    # Step 1: Replace common Unicode punctuation with ASCII equivalents
    # Smart quotes
    ...
    # Step 2: Remove remaining non-ASCII characters (aggressive filtering)
    # This prevents any exotic Unicode from reaching the tokenizer
    try:
        text = text.encode('ascii', errors='ignore').decode('ascii')
    except Exception as e:
        logger.warning(f"Error during ASCII encoding: {e}")
        # Fallback: remove non-printable only
        text = "".join(
    ...
    # Step 4: Remove any remaining control characters except newlines
...
def _split_long_segment(segment: str, max_length: int) -> List[str]:
    """
    Splits a segment that exceeds max_length into smaller chunks at word boundaries.
    This is a fallback for when a single sentence is too long.
    
    Args:
        segment: The text segment to split.
        max_length: Maximum length for each chunk.
    
    Returns:
        List of chunks, each respecting max_length.
    """
    ...
    for word in words:
        word_len = len(word)
        # +1 for the space
        needed_length = current_length + (1 if current_chunk else 0) + word_len
        ...
        if needed_length <= max_length:
            ...
            # Word itself might be longer than max_length
            if word_len > max_length:
                # Split the word into character chunks (last resort)
...
def chunk_text_by_sentences(
    ...
    and STRICTLY enforcing a maximum chunk character length. Sentences longer than chunk_size
    will be split at word boundaries.

    Args:
        full_text: The complete text to be chunked.
        chunk_size: The STRICT maximum character length for each chunk.
                    No chunk will exceed this length.

    Returns:
        A list of text chunks, ready for TTS. Each chunk is guaranteed to be <= chunk_size.
    """
    if not full_text or full_text.isspace():
        return []
    if chunk_size <= 0:
    ...
    for (
        ...
        # STRICT ENFORCEMENT: If a single segment exceeds chunk_size, split it
        if segment_len > chunk_size:
            logger.warning(
                f"Segment (length {segment_len}) exceeds chunk_size {chunk_size}. "
                f"Splitting at word boundaries."
            )
            ...
            # Split the long segment and add each part as a chunk
            split_parts = _split_long_segment(segment_text, chunk_size)
            text_chunks.extend(split_parts)
            continue

        # Normal case: try to add segment to current chunk
    ...
    if current_chunk_sentences:
        ...
        if len(final_chunk) > chunk_size:
            logger.warning(
                f"Final chunk (length {len(final_chunk)}) exceeds chunk_size. Splitting."
            )
            text_chunks.extend(_split_long_segment(final_chunk, chunk_size))
        else:
    ...
    if not text_chunks and full_text.strip():
        logger.warning(
            "Text chunking resulted in zero chunks despite non-empty input. "
            "Splitting full text with strict size limit."
        )
        return _split_long_segment(full_text.strip(), chunk_size)
    
    # Verify all chunks are within limit
    for i, chunk in enumerate(text_chunks):
        if len(chunk) > chunk_size:
            logger.error(
                f"CRITICAL: Chunk {i+1} has length {len(chunk)}, exceeds limit {chunk_size}. "
                f"This should not happen and indicates a bug in chunking logic."
            )

    logger.info(
        f"Text chunking complete. Generated {len(text_chunks)} chunk(s). "
        f"Max chunk length: {max(len(c) for c in text_chunks) if text_chunks else 0}"
    )
    return text_chunks


# --- File System Utilities ---
def get_valid_reference_files(ref_audio_dir_path: Optional[Path] = None) -> List[str]:
    """
    Scans the configured reference audio directory and returns a sorted list of
    valid audio filenames (.wav, .mp3).
    """
    if ref_audio_dir_path is None:
